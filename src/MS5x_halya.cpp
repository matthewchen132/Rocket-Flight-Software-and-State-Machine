...
{
    if (_PRESS != 0 and _PTYPE != 0)
    { // Only convert if a new type is specified and Pressure has a value
        switch (_PTYPE)
        {
        case 0:
            break; // Just including this for safety
        case 1:    // mbar to Pa
            _PRESS *= 100.0f;
...
void MS5x::setTempC()
{
    if (_TEMP != 0 and _TTYPE != 0)
    { // Only convert if a new type is specified and temp has a value
        switch (_TTYPE)
        { // Convert from existing units to C
        case 0:
            break; // Just including this for safety
        case 1:
...
bool MS5x::checkCRC()
{
    ...
    if ((Calc_CRC4(_COEFFICENTS) == Read_CRC4()))
        statusRet = true;
    return statusRet;
}
...
void MS5x::checkUpdates()
{
    ...
    if ((uint32_t)(currMillis - _PREVSTEPTIME) >= _READDELAY)
    {
        ...
        case 2: // Send command to read Temperature then do conversion math
            ...
            { // Pressumably everything is fine, continue with conversion process
                Readout();
                _READSTEP += 1;
                _PREVSTEPTIME = currMillis;
            }
...
void MS5x::Readout()
{
    ...
    case 0:
        ...
        break; // Pascals
    case 1:
        _PRESS = pressure * 0.01f;
        break; // Millibars (also Hectioascals)
    case 2:
        _PRESS = pressure / 3386.3886667;
        break; // Inches Hg
    }
...
double MS5x::getAltitude(bool temperatureCorrected)
{
    ...
    if (temperatureCorrected)
    {
        ...
        {
            if (_TTYPE != 2)
            { // Only convert if temperature isn't already in Kelvin
                switch (_TTYPE)
                { // Convert from existing units to C
                case 0:
                    temperature += 273.15f;
                    break; // C to K
                case 1:
                    temperature = (_TEMP + 459.67) * 5.0f / 9.0f;
                    break; // F to K
                case 2:
                    break; // This is here just for safety
                }
            }
            else
            {
